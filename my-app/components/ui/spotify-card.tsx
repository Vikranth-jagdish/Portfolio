...
export default function SpotifyCard() {
    ...
    const isDataPlaying = data?.isPlaying;
    const currentSong = isDataPlaying ? {
        title: data.title,
        artists: data.artist,
        duration: 240, // Mock duration for visuals
        ...
        artists: "Spotify",
        duration: 0,
        albumArt: "https://images.unsplash.com/photo-1614680376593-902f74cf0d41?q=80&w=1000&auto=format&fit=crop", // placeholder
        url: "#"
    };

    const songDuration = currentSong.duration || 1;

    // Sync playing state from API
    useEffect(() => {
        if (data) {
    ...
    useEffect(() => {
        ...
        if (isPlaying && !isDraggingProgress && isDataPlaying) {
            interval = setInterval(() => {
                setAnimationProgress(prev => {
                    if (prev >= 100) {
                        return 0; // Just loop visual progress
                    }
                    return prev + 0.2; // slow visual progress
                });
            }, 100);
        }
        return () => clearInterval(interval);
    }, [isPlaying, isDraggingProgress, songDuration, isDataPlaying]);
    ...
    const handleProgressMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        const onMouseMove = (e: MouseEvent) => handleProgressChange(e);
        const onMouseUp = () => {
    ...
    const togglePlay = (e?: React.MouseEvent) => {
        e?.stopPropagation();
        // In live mode, this is just visual
            ...
            >
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        <button className="w-7 h-7 rounded-full flex items-center justify-center hover:bg-white/10 transition-all duration-300">
